...
pub fn load() -> LinderaResult<Dictionary> {
    ...
    if let Ok(dict_path) = std::env::var("LINDERA_IPADIC_PATH") {
    ...
    Err(LinderaErrorKind::Io.with_error(anyhow::anyhow!(
        "IPADIC dictionary not found. Please set LINDERA_IPADIC_PATH environment variable or place dictionary files in one of these locations: {}",
        search_paths.join(", ")
    )))
}
