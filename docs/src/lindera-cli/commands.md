...
- `--space-penalty`: Enable the left-space penalty (mecab-ko's `left-space-penalty-factor`) with the rules shipped in the dictionary's `metadata.json` (ko-dic ships mecab-ko-dic's). A candidate that starts right after whitespace and whose first part-of-speech tag is listed gets the cost added. Errors when the dictionary ships no rules. Default: off. See the [Segmenter](../lindera/segmenter.md#left-space-penalty-korean) documentation
- `--space-penalty-rules`: Enable the left-space penalty with explicit rules as JSON instead of the dictionary's, e.g. `'{"rules":[{"pos":["JKS","JX"],"cost":6000}]}'`. Mutually exclusive with `--space-penalty`
