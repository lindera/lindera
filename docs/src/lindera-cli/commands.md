...
- `--max-grouping-len`: Cap on unknown-word grouping, in characters beyond the first (MeCab's `max-grouping-size`; MeCab defaults to 24). The cap is applied at each position: a same-category run longer than the cap is not grouped there, the single-character candidate (plus the length ladder and dictionary words) remains, and the remaining tail is grouped again once it fits, so no unknown token exceeds cap+1 characters. `0` or omitted: unbounded. See the [Segmenter](../lindera/segmenter.md#unknown-word-grouping) documentation for an example
...
- `--disable-space-penalty`: Disable the left-space penalty (mecab-ko's `left-space-penalty-factor`) that a dictionary shipping rules in its `metadata.json` applies by default (ko-dic ships mecab-ko-dic's). A candidate that starts right after whitespace and whose first part-of-speech tag is listed gets the cost added; disabling it restores the v6.0 output for Korean. See the [Segmenter](../lindera/segmenter.md#left-space-penalty-korean) documentation
