...
segmenter:
  ...
  # space_penalty: { rules: [ { pos: [JKS, JX], cost: 6000 } ] } # left-space penalty (Korean); off by default
...
| `space_penalty` | object | *(off)* | Left-space penalty rules, `{"rules": [{"pos": [...], "cost": n}, ...]}` (mecab-ko's `left-space-penalty-factor`): a candidate that starts right after whitespace and whose first part-of-speech tag is listed gets the cost added. `false` or `null` leaves it off. See [Segmenter](../lindera/segmenter.md#left-space-penalty-korean) |
