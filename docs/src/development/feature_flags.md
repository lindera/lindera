...
- The `mmap` feature enables memory-mapped dictionary loading for filesystem-based dictionaries, requested via `--mmap` (CLI) or the `use_mmap` segmenter config key. The word-list files (`dict.vals`/`dict.wordsidx`/`dict.words`) and the connection-cost matrix (`matrix.mtx`) stay lazily paged and cost no anonymous memory; only the double-array trie is still fully materialized. It has no effect on embedded dictionaries, whose connection-cost matrix is read in place from the binary regardless.
