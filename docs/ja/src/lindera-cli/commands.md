...
- `--space-penalty-rules`: 左側空白ペナルティ（mecab-ko の `left-space-penalty-factor`）をルールの JSON 指定で有効にする。例: `'{"rules":[{"pos":["JKS","JX"],"cost":6000}]}'`。直前に空白があり、先頭品詞タグが一覧にある候補にコストを加算する。デフォルト: オフ。mecab-ko-dic のルールは [Segmenter](../lindera/segmenter.md#左側空白ペナルティ韓国語) のドキュメントを参照
...
- `--context-id-freq` / `-f`: 接続コストIDの並び替えに使用するコンテキストID アクセス頻度ファイル（オプション。辞書の `metadata.json` で `connection_id_mapping: true` が設定されている場合にのみ意味を持ちます）
...
ファイルフォーマットや高度な機能の詳細については、[学習パイプライン](../development/training_pipeline.md) を参照してください。
...
- `matrix.def`: 全 (right\_id, left\_id) ペアを網羅する密な接続コスト行列
- `unk.def`: 未知語定義
- `char.def`: 文字種定義
...
3. **既存の設定を保持**します（辞書名、文字エンコード設定、スキーマ定義、その他のユーザー定義設定など）
