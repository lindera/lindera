...
ハングルの数詞は他の形態素と同形です（`이`は主格助詞、`만`は補助詞でもあります）。すべてのトークンを変換すると通常の文が壊れるため（`이것은 사과입니다`が`2것 은 4과 입니다`になる）、既定では数詞の品詞タグに限定しています。漢数字も変換する場合は`SH`を追加してください。ただしko-dicは一部の漢数字を`NNG`と判定するため、対応は部分的です。
