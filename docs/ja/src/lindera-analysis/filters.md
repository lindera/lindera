...
IPADICに対する`japanese_compound_word`のように変換前にトークンを結合する処理は[#1026](https://github.com/lindera/lindera/issues/1026)で対応予定です。漢字表記の`二千二十六/SH`のように数値全体が1トークンの場合はそのまま変換されます。
