...
トークンの表層形を、辞書の`base_form`（または`orthographic_base_form`）フィールドに登録された原形（辞書形）に置き換えます。動詞・形容詞のレンマ化（見出し語化）として機能します。未知語処理によって生成されたトークン（`token.word_id.is_unknown()`）は変更されません。
...
結合後のトークンの詳細には、辞書スキーマから解決した品詞の位置に `new_tag` が入ります。それより前の品詞以外のフィールド（SudachiDict の `display_surface`）と残りのフィールドはすべて `*` になります。
...
タグは 4 階層のカンマ区切りに正規化され（不足分は `*` で補完）、各トークンの品詞詳細 4 つ（辞書スキーマで `part_of_speech` という名前のフィールドから始まる 4 つ。スキーマにそのフィールドが無い場合は先頭の詳細から）と完全一致で比較されます。IPADIC の助詞トークンは必ず `助詞,係助詞` のようにサブカテゴリを持つため、`助詞` 単独では一致しません。一方、助動詞はサブカテゴリを持たないため（`助動詞,*,*,*`）、`助動詞` 単独で一致します。この動作は `japanese_stop_tags`、`japanese_compound_word`、および `japanese_number` の `tags` も同様です。位置はスキーマ経由で解決されるため、SudachiDict の `display_surface` のように品詞の前に列があるスキーマでも同じ規則で一致します。
...
トークンは、その全ての文字が数詞である場合にのみ変換されます（Lucene の `JapaneseNumberFilter` と同じ規則）。そのため、数詞で始まるだけの通常の語は変換されません。`一部`は`1部`にならず`一部`のままです。

`japanese_compound_word`の後段で使う場合は、`名詞,数`のトークンだけを結合してください。助数詞（`名詞,接尾,助数詞`）を数値トークンに結合すると、そのトークンは数詞ではなくなるため、`一万円`は変換されずそのまま残ります。

**パラメータ:**

| パラメータ | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `tags` | array\<string\>または`null` | いいえ | 変換対象を限定する品詞タグ（カンマ区切りで最大4階層）。`japanese_keep_tags` と同じ規則で一致判定される。省略または`null`の場合はすべてのトークンが変換対象になる |
...
トークンの表層テキストを、辞書の`reading`フィールドに登録された読み（カタカナ）に置き換えます。未知語処理によって生成されたトークン（`token.word_id.is_unknown()`）は変更されません。
...
最初の品詞タグが`tags`のいずれかに一致する連続した韓国語トークンを、1つの複合語トークンに結合します。結合後のトークンは先頭トークンの`byte_start`と`position`、末尾トークンの`byte_end`を持ち、`position_length`は結合したトークン数になります。隣接トークンが一致しない単独のトークンはそのままです。

ko-dicは漢数詞を形態素単位に分割するため、`이/NR 천/NR 이/NR 십/NR 육/NR`を`korean_number`で変換する前に1トークンへ戻すのがこのフィルタの役割です。

| 入力 | ko-dicのトークン | `korean_compound_word`適用後（`tags: ["SN", "NR"]`、`new_tag: "NR"`） | `korean_number`適用後 |
| --- | --- | --- | --- |
...
| パラメータ | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `tags` | array\<string\> | はい | 結合対象となるトークンを示す品詞タグ（`NR`や`SN`などko-dicの単一タグ） |
| `new_tag` | string | いいえ | 結合後のトークンに付与する品詞タグ。他のdetailは`*`になる。省略した場合は`복합어`が付与されるが、これはko-dicのタグではないため、後続の`korean_number`や`korean_keep_tags`に結合後のトークンを渡す場合は`NR`などのタグを指定する |
...
トークンの表層テキストに含まれる漢数詞由来の韓国語の数値表現（ハングル数字、漢数字、全角数字）をアラビア数字に変換します。固有語数詞（`하나`、`둘`、`열`、`스물`など）は位取りの構造を持たないため変換しません。

変換はトークンごとに独立して行われます。ko-dicは漢数詞を形態素単位に分割するため、1つの数を構成するトークン群が1つのアラビア数字にまとまるわけではありません。

| 入力 | ko-dicのトークン | `korean_number`適用後 |
| --- | --- | --- |
| `이천이십육년` | `이/NR 천/NR 이/NR 십/NR 육/NR 년/NNBC` | `2 1000 2 10 6 년` |
| `10만` | `10/SN 만/NR` | `10 10000` |

IPADICに対する`japanese_compound_word`のように、変換前にトークンを結合するには`korean_compound_word`をこのフィルタの前に置きます。結合後のトークンを変換対象にするため、`new_tag`にはこのフィルタの`tags`に含まれるタグを指定します。
...
このチェーンで`이천이십육년`は`2026 년`、`10만`は`100000`、`2천26`は`2026`になります。漢字表記の`二千二十六/SH`のように数値全体が1トークンの場合は、このフィルタ単体でそのまま変換されます。

**パラメータ:**

| パラメータ | 型 | 必須 | 説明 |
| --- | --- | --- | --- |
| `tags` | array\<string\>または`null` | いいえ | 変換対象を限定する品詞タグ。省略した場合は`["SN", "NR"]`、`null`の場合はすべてのトークンが変換対象になる |

トークンは、その全ての文字が数詞である場合にのみ変換されます（Lucene の `KoreanNumberFilter` と同じ規則）。これにより固有語数詞が保護されます。`일곱`（7）は漢数詞の`일`で始まるため、1文字ずつ変換すると`1곱`になってしまいます。

ハングルの数詞は他の形態素とも同形で（`이`は主格助詞、`만`は補助詞）、これらは1文字なので全文字一致の規則では区別できません。そのため既定では数詞の品詞タグに限定しています。

漢数字も変換する場合は`SH`を追加してください。対応は2つの意味で部分的です。ko-dicは漢字語全体を`NNG`と判定するため`參加`や`萬歲`は変換されず、一方で単語に結び付かない単独の漢字は`SH`となるため、`十字架`は`10 字 架`、`百貨店`は`100 貨 店`になります。
...
トークンの表層テキストを、辞書の`reading`フィールドに登録された読みに置き換えます。未知語処理によって生成されたトークン（`token.word_id.is_unknown()`）は変更されません。
