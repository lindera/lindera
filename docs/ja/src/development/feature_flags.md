...
- `mmap` feature はファイルシステム辞書に対するメモリマップド読み込みを有効にします（CLIの`--mmap`または`segmenter`設定の`use_mmap`キーで要求）。単語リストファイル（`dict.vals`/`dict.wordsidx`/`dict.words`）と接続コスト行列（`matrix.mtx`）は遅延読み込みされ、匿名メモリを消費しません。常に全体がメモリに展開されるのはダブル配列トライのみです。埋め込み辞書には影響しませんが、埋め込み辞書の接続コスト行列もバイナリ内から直接参照されます。
