...
- `mmap` feature はファイルシステム辞書に対するメモリマップド読み込みを有効にします（CLIの`--mmap`または`segmenter`設定の`use_mmap`キーで要求）。実際に遅延読み込みされるのは最大の単語リストファイル（`dict.vals`/`dict.wordsidx`/`dict.words`）のみで、接続コスト行列とダブル配列トライは常に全体がメモリに展開されます。埋め込み辞書には影響しません。
