...
| `analysis` | 分析チェーン: character filter・token filter・`Tokenizer` | 無効 |
| `train` | CRF ベースの辞書学習（`lindera-trainer` に依存） | CLI のみ |

- `mmap` はメインの `lindera` クレートでデフォルトで有効です。
- `analysis` はデフォルトでは無効です（v5.0 以降）。デフォルトビルドは
  `Segmenter` API を中心とした純粋な形態素分割ビルドで、外部の分析パイプラインの
  背後でセグメンテーションバックエンドとして Lindera を使う場合に有用です。
  `analysis` を有効にすると、`character_filter`・`token_filter`・`tokenizer`
  モジュールと、その依存クレート（kanaria、regex、serde_yaml_ng、unicode-blocks、
  unicode-normalization、unicode-segmentation、およびフィルタでの daachorse 使用）が
  追加されます。
