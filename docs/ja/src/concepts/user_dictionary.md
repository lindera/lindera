...
`base_form`フィールド（CSV/詳細情報の7番目のフィールド）が表層形ではなく`*`になっている点に注意してください。
シンプルな3カラムのユーザー辞書スキーマでは `surface`、`part_of_speech`、`reading` の3つしか指定できず、
`base_form` を含むそれ以外のフィールドはすべて辞書の `metadata.default_field_value`
（IPADICの場合は`*`）で埋められます。
