...
**役割**: 素性テンプレート定義
**フォーマット**: MeCab互換の素性抽出パターン

```text
# ユニグラム素性（単語レベルの素性）
UNIGRAM U00:%F[0]           # 品詞（素性要素0）
UNIGRAM U01:%F[0],%F?[1]    # 品詞+品詞細分類（%F?[n] = オプション、* の場合スキップ）
UNIGRAM U02:%F[6]           # 原形
UNIGRAM U03:%w              # 表層形（メタ文字）

# バイグラム素性（文脈組み合わせ素性）
BIGRAM B00:%L[0]/%R[0]      # 左品詞 / 右品詞
BIGRAM B01:%L[0],%L[1]/%R[0],%R[1]  # 左品詞細分類 / 右品詞細分類
```

- **目的**: どの情報から素性を抽出するかを定義
- **フォーマット**: 各行は `UNIGRAM` または `BIGRAM` で始まり、ラベル（例: `U00:`, `B00:`）とテンプレートパターンが続く
- **テンプレート変数**:
  - `%F[n]` / `%F?[n]`: 素性フィールドのインデックス n（`?` = オプション、値が `*` の場合スキップ）
  - `%L[n]`: 左文脈の素性フィールドのインデックス n（rewrite.def の left セクションで変換後）
  - `%R[n]`: 右文脈の素性フィールドのインデックス n（rewrite.def の right セクションで変換後）
  - `%w`: 単語の表層形
  - `%u`: ユニグラムリライト済み素性文字列（rewrite.def の unigram セクションで変換後）
  - `%l`: 左文脈リライト済み素性文字列（rewrite.def の left セクションで変換後）
  - `%r`: 右文脈リライト済み素性文字列（rewrite.def の right セクションで変換後）

#### 6. **素性リライトルール (rewrite.def)**

**役割**: CRF学習前の素性正規化のためのリライトルール
**フォーマット**: MeCab互換の3セクション形式（空行で区切り）

ファイルは空行で区切られた3つのセクションで構成されます：

1. **ユニグラムセクション**: ユニグラムテンプレート（`%F[n]`, `%u`）の素性をリライト
2. **左文脈セクション**: 左文脈（`%L[n]`, `%l`）の素性をリライト
3. **右文脈セクション**: 右文脈（`%R[n]`, `%r`）の素性をリライト
...
# セクション2: 左文脈リライトルール（上の空行でセクション区切り）
名詞,固有名詞,*  名詞,固有名詞
助詞,*  助詞
*  *

# セクション3: 右文脈リライトルール（上の空行でセクション区切り）
名詞,固有名詞,*  名詞,固有名詞
助詞,*  助詞
*  *
```

- **目的**: 素性を正規化してスパース性を軽減し、学習効率を向上させる
- **フォーマット**: 各行は `パターン\t置換` のタブ区切り。パターンでは `*` をワイルドカードとして使用可能
- **マッチング**: パターンは前方一致で照合され、各セクションで最初にマッチしたルールが適用される
- **効果**: ユニグラム・左文脈・右文脈に対して異なるリライトルールを適用でき、きめ細かな素性正規化が可能
