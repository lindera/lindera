...
タグは 4 階層のカンマ区切りに正規化され（不足分は `*` で補完）、各トークンの品詞詳細 4 つ（辞書スキーマで `part_of_speech` という名前のフィールドから始まる 4 つ。スキーマにそのフィールドが無い場合は先頭の詳細から）と完全一致で比較されます。IPADIC の助詞トークンは必ず `助詞,係助詞` のようにサブカテゴリを持つため、`助詞` 単独では一致しません。一方、助動詞はサブカテゴリを持たないため（`助動詞,*,*,*`）、`助動詞` 単独で一致します。
