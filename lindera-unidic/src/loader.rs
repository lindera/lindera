...
pub fn load() -> LinderaResult<Dictionary> {
    ...
    if let Ok(dict_path) = std::env::var("LINDERA_UNIDIC_PATH") {
    ...
    Err(LinderaErrorKind::Io.with_error(anyhow::anyhow!(
        "UniDic dictionary not found. Please set LINDERA_UNIDIC_PATH environment variable or place dictionary files in one of these locations: {}",
        search_paths.join(", ")
    )))
}
